...
class R2Analyzer {
  ...
  /**
   * Process an individual object and extract statistics
   */
  private processObject(key: string, size: number): void {
    // Parse path: ticks/{SYMBOL}/{YYYY}/{MM}/{DD}/part-{timestamp}.json
    ...
    // Initialize stats for new symbol
  ...
  async displayResults(sample: boolean = false): Promise<void> {
    ...
    for (const symbol of sortedSymbols) {
      ...
      // Sample actual tick count if requested
      if (sample) {
    ...
    // Show monthly breakdown for each symbol
    console.log('\n📅 Monthly Breakdown:');
    for (const symbol of sortedSymbols) {
      const stats = this.stats.get(symbol)!;
      console.log(`\n  ${symbol}:`);
